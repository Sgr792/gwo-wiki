...
6. 位置适配只缩放 translation 曲线。四元数旋转必须保持单位长度，不能把 W/X/Y/Z 分量整体乘以 0.25；缩放曲线通常保持 1。
7. 每个动作的名称必须唯一且稳定，不要出现 `idle`、`idle.001`、`Action` 等无意生成的动作。
8. 动画源文件中不存在的动作不要在 JSON 中虚构；JSON 中启用的动作必须能在 `.anim.glb` 中找到。
9. 动画开始帧、结束帧、事件帧、提交帧和声音帧必须在同一个时间基准上。
10. 动画文件只负责姿态。弹量、伤害、换弹提交和服务器判定仍由配置与游戏逻辑决定。
...
- 步枪 `empty_additive` 只控制 `j_bolt`、`j_boltrel`。
- 手枪 `empty_additive` 只控制 `j_slide`、`j_barrel`、`j_hammer`。
- 霰弹枪 `empty_additive` 控制配置指定的管内弹与 follower。
- `bullet_additive` / `shell_additive_*` 只控制配置中的 `j_ammo_*` 与 follower。
- `aim_in` / `aim_out` 只控制 `tag_ads`。
- `aim_additive` 只控制 `tag_weapon`。

内容作者修改 `bone_mask` 后，动画通道必须同步服从新的所有权范围。
...
- 普通换弹与瞄准换弹同帧切换时，应混合两套“手相对 `tag_weapon`”的姿态，不能先混合世界姿态再计算相对矩阵。
...
- 左手或右手最后一帧与 idle 数值相同仍可能因父级空间不同而跳动，应检查局部矩阵和父级变换，而不只比较曲线数值。
- 切枪后重新拿回尚未完成 rechamber 的武器，状态机会在 `draw` 完成后恢复待完成动作；动画文件不能自行伪造完成状态。
...
- 奔跑动画应按对应源动作正确叠加；不使用的左右手 IK 层不要混入导出结果。
...
- 手臂长度适配优先在动作中有意识地制作，不改变独立手臂模型的原版尺寸。
- 不要把四元数旋转曲线当作普通比例数值缩放；需要改变动作幅度时应使用姿态混合或在 Blender 中重定向后烘焙。
...
- 没有相机运动的动作应删除 `tag_camera` 通道，不要保留 46 个静态骨骼式的批量烘焙结果。
...
- 一次导入一个动画文件时，应把其中的通道按骨骼名称分配给所有拥有目标骨骼的 Armature，但最终仍只生成一个动作名称。
- 导入器不能因为选择了一个文件就自动导入整个文件夹，除非明确启用批量模式。
- 导入后不要自动保存 `.blend`，先由作者检查姿态和通道。
- 导出时以主骨架作为动作时间轴来源，次级骨架只贡献其拥有的骨骼通道。
- `GUI_ARMATURE_NAMES` 不能只写主骨架后忽略枪管、弹匣、护木等次级骨架。
- 骨骼所有权不明确时必须在 `GUI_BONE_OWNER_OVERRIDES` 中显式指定。
- 同名非空 NLA 轨道用于合并同一动作；没有参与该动作的次级骨架不应强制生成静态轨道。
...
- 游戏内测试 idle、瞄准、射击、最后一发、空击、四类换弹、检视、奔跑、近战和切枪。
