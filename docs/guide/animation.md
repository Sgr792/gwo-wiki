...
5. 在绑定和制作动画之前统一对象变换；已绑定或已制作动画的工程，不要单独应用 Armature 的旋转和缩放。确需修改时同步更新受影响的模型、绑定姿态和动画并重新检查。
...
variant、sequence、marker、Pose Graph 和打断先阅读[配置示例](./config-examples.md)，管式装填参考本页帧事件说明。如已持有匹配版本的默认包，也可额外对照：
...
分支、序列、标记、打断和姿态层先看[独立配置示例](./config-examples.md)。匹配版本的默认包可作为额外参考，但不是学习本教程必须持有的文件。
